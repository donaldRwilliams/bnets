...
ppc_plot <- function(x, nodes, alpha, color_y_rep, color_y){
  ...
  #n_row_X  <- nrow(d)
  names_d  <-  rep(colnames(x$data[nodes]), each = length(x$data[,1]))
  names_plt <-  rep(colnames(x$data[nodes]), each = 1)
  dat_y$L1 <- names_d
